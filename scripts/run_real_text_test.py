#!/usr/bin/env python
"""端到端运行Pipeline在真实样本的文本切片上（前20页无图版）。"""
import sys
import os
import time
import json
...
def main():
    ...
    pdf = repo_root / "eval_real_test" / "测试文档1_20p_text.pdf"
    out = repo_root / "eval_real_test"
    out.mkdir(parents=True, exist_ok=True)

    start = time.time()
    print(f"[{time.time()-start:.0f}s] PDF: {pdf} ({pdf.stat().st_size/1024:.1f} KB)", flush=True)
    ...
    for h in headings[:10]:
        print(f"  L{h.global_level}: {h.text[:60]!r}", flush=True)
